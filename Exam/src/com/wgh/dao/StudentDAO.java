        ...
        String ID="";
            try {
                if (rs.next()) {											//假如存在记录
                	...
                	/********************************************************************************/
              		sql = "INSERT INTO tb_student (ID,name,pwd,sex,joinTime,question,answer,profession,cardNo) values('" +
                                 ID+ "','" +s.getName() +"','"+s.getPwd()+"','"+s.getSex()+"','"+new SimpleDateFormat("yyyy-MM-dd HH:mm:ss").format(date)+
                                 "','"+s.getQuestion()+"','"+s.getAnswer()+"','"+s.getProfession()+"','"+s.getCardNo()+"')";
