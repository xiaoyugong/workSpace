...
public class KMeansSeImpl
{
	...
	protected String strInPathC = Const.HDFS_MYCLU + Const.INPATH + Const.SPLIT_PATH;
	...
	//	初始化数据来源
	public KMeansSeImpl(DMEntityCondition cond)
	{
		if (null != cond)
		{
		    strInPath += cond.getDataSet();
		    strInPathC += cond.getKMeansCenter();
	...
	{
		{
			...
			//Path inputCenter = new Path(strInPathC);
			...
			//paraDm.put("-i",input.toString());   //  used for input file is sequence
		    ...
		    paraDms.put("-i",strInPath);   //  used for input file is sequence
		    paraDms.put("-c", strInPathC);	//	初始聚类中心文件，上一步聚类的结果
