...
public class BaseDaoSupport<T> implements BaseDao<T> {
	...
	public synchronized PageBean<T> pageQuery(final String hql, int pageSize,int page) {
		 ...
		 hibernateTemplate.execute(new HibernateCallback() {
			 public Object doInHibernate(Session session) throws HibernateException, SQLException {
				 for(int i = 0; i < list.size(); i++){
					 T t = list.get(i);
					 session.save(t);
					 if(list.size() < 20 && i == list.size()-1 || i%20 == 0&&i!=0){
						 session.flush();
						 session.clear();
					 }
				 }
	...
	public List bulkFind(final String hql,final Object[] obj) {
		Object list = hibernateTemplate.execute(new HibernateCallback() {
			 public Object doInHibernate(Session session) throws HibernateException, SQLException {
						Query query = session.createQuery(hql);
						query.setParameterList("in", obj);
						return	query.list();
	         }
	     });
		return (List) list;
