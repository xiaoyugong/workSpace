...
//		lastJobId=jobId;
		...
		monitorJobs=new LinkedHashMap<String,JobInfo>();
		monitorJobs.put(jobname1, new JobInfo(jobname1,System.currentTimeMillis()-5000));
		monitorJobs.put(jobname2, new JobInfo(jobname2,System.currentTimeMillis()-5000));
		monitorJobs.put(jobname3, new JobInfo(jobname3,System.currentTimeMillis()-5000));
		lastJob=jobname3;
	...
	public static JobStatus[] getNewJobStatus() throws IOException{
		...
		JobStatus[] jobStatus = new JobStatus[3];
		jobStatus[0] = new JobStatus();
		jobStatus[1] = new JobStatus();
		jobStatus[2] = new JobStatus();
		Iterator<Entry<String, JobInfo>> iter = monitorJobs.entrySet().iterator();
		int k = 0;
		while (iter.hasNext()) {
			...
			long tmp =0;
			for(int i=0;i<jobStatusAll.length;i++){
				if(key.equals( jobStatusAll[i].getJobName())){
					if (jobStatusAll[i].getStartTime() > tmp) {
						tmp = jobStatusAll[i].getStartTime();
						f=i;
					}
				}
			}
			if(f != 0){
				jobStatus[k++] = jobStatusAll[f];
			}
		}
		return jobStatus;
	}
